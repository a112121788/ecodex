...
| Restore | `session.json`、`resume.json`、trusted binding、安全过滤已可用；关闭窗口后保留后台终端与重开接回已在 S1 补齐 | `03-data-and-ipc.md`、`07-implementation-backlog.md` |
...
目标：让 ECodex 窗口关闭、后台运行和任务栏激活行为可预测，避免重复主进程和后台状态误解。

| Outcome | 说明 | 指标 |
|---|---|---|
| X 关闭主进程 | 右上角 X 直接退出 `ecodex-app`，并按设置保留或终止 daemon 托管终端 | `ecodex-app` 不再因 X 关闭留在后台 |
| 最小化后台运行 | 最小化保留任务栏按钮，后台终端与通知继续工作 | 托盘可恢复窗口，可选择保留或终止后台终端 |
| 单实例激活 | 任务栏按钮 / 固定图标激活已有窗口，安装器快捷方式与进程 AUMID 一致 | 不产生第二个长期存活的 `ecodex-app` 实例 |
| 命令生命周期通知 | 默认 PowerShell hook 回传命令开始、结束、退出码，后台按完成/失败/等待输入提醒 | 非激活或隐藏状态下低噪声通知，前台不刷 Toast |
| 开箱 skills | 预制 skills 从包内模板安全种子到 `%USERPROFILE%\.agents\skills` | 同名目录跳过，不覆盖用户已有 skill |
| 终端多行输入友好 | `Ctrl+Enter` 在所有终端中输入换行，`Enter` 仍提交 | PowerShell / cmd / Codex CLI 手测通过 |
...
| R-07 | 默认 shell integration 修改用户 profile | High | profile 标记块冲突、备份失败、用户自定义内容被误改 | 只写 ECodex 标记块；写入前备份；status/uninstall 可检查和移除；冲突时跳过并提示 |
| R-08 | 通知过度打扰 | Medium | dev server 或测试日志产生大量 Toast | 只在隐藏/非激活且命中关键生命周期状态时通知；加入去重与冷却 |
