...
| `STATUS` | — | 返回 `{version, workspaces, selectedWorkspace, unreadNotifications}`（`version` 当前为 `1.0.2` 程序集版本） |
...
当前源码包含纯 Core 的 `AgentConversationStoreService`、`AgentConversationThread`、`AgentConversationMessage` 与 `AgentConversationRecorder` seam。该存储只读写调用方显式传入的 agent 根目录；recorder 只包装规范化 thread/message 写入；Session Vault 已通过 App 层 provider 读取 app-owned agent root，`AgentRuntimeService` 与真实 Agent 消息写入仍未落地。
...
- `AgentConversationRecorder`：
  - `StartThread` — 接收 `AgentConversationThreadRecordingInput`，创建 workspace-scoped thread
  - `AppendMessage` — 接收 `AgentConversationMessageRecordingInput`，只允许 `user / assistant / tool / system`
  - token 缺失默认传 `0`；`IsCompaction=true` 只允许 `system / assistant`
  - 写入失败返回 `AgentConversationRecordingResult.Succeeded=false`，不向 terminal / UI 主流程抛异常
...
| Agent 会话 | `AgentConversationStoreService` + `AgentConversationRecorder` + `AgentConversationFailureLoopEvidenceProvider` + App 层 provider | 串联 Agent 消息、压缩与 token 统计摘要 | Core recorder seam 与 Session Vault provider 接线已落地，但 runtime 写入未接 App / LLM；只能使用 app-owned / 调用方显式传入的 agent 根目录，不扫描其他 profile |
...
Agent runtime 写入尚未接入 App / LLM。当前 `AgentConversationRecorder` 已提供纯 Core seam；后续 runtime 必须先遵守以下契约，再把规范化事件交给 recorder / `AgentConversationStoreService`：
...
| Core seam | 契约 |
|---|---|
| `AgentConversationThreadRecordingInput` | `workspaceId` 必填；`surfaceId / paneId / title / threadId / createdAtUtc` 可选；不携带 provider 或 tool schema |
| `AgentConversationMessageRecordingInput` | `threadId / role / content` 是最小输入；`messageId / createdAtUtc / token metadata / IsCompaction` 可选 |
| `AgentConversationRecorder` | 只校验 role、compaction 与 token 默认值，再调用 store；store 异常转为 `AgentConversationRecordingResult` 失败，不抛给调用方 |
| 非目标 | 不读取 `.env*`、`config/credentials*`、`secrets/**`，不调用 LLM，不接 UI，不扫描 `%USERPROFILE%` 下其他 profile |
