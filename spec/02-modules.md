...
| `Services/FailureLoopEvidenceCollector.cs` | `FailureLoopEvidenceCollector / IFailureLoopEvidenceSourceProvider / CommandLogFailureLoopEvidenceSourceProvider` | OBS-01 provider seam；由调用方注入命令日志、transcript 内容加载和 daemon log provider，collector 只为失败时间窗内 transcript 加载内容，不直接 new `CommandLogService` |
