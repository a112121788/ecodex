...
[![CI](https://github.com/a112121788/ecode/actions/workflows/ci.yml/badge.svg)](https://github.com/a112121788/ecode/actions/workflows/ci.yml)
[![Docs](https://github.com/a112121788/ecode/actions/workflows/docs.yml/badge.svg)](https://github.com/a112121788/ecode/actions/workflows/docs.yml)
[![License: MIT](https://img.shields.io/badge/license-MIT-blue.svg)](LICENSE)

ECode 是一款面向 Windows 的原生 SuperTerminal：用 WPF + ConPTY 构建，把多项目终端、标签页、分屏、Browser Surface、命令日志、会话恢复和脚本化自动化放在同一个键盘优先工作台里。

- 文档站点：https://a112121788.github.io/ecode/
- 安装指南：https://a112121788.github.io/ecode/installation
- CLI 参考：https://a112121788.github.io/ecode/cli
- Browser API：https://a112121788.github.io/ecode/browser-api

> 主程序为 `ecode-app.exe`，CLI 为 `ecode.exe`，后台守护进程为 `ecode-daemon.exe`。
...
| 需要让外部脚本驱动终端 UI | 命名管道 CLI API 支持通知、workspace、surface、split、status | `ecode notify`、`ecode split right` |
...
- 命令日志、命令历史和 `ecode.json` 项目命令，让常用操作可审计、可回放、可复用。
- Browser Surface 基于 WebView2，并暴露 snapshot、click、fill、eval、screenshot 等自动化能力。
...
更多教程见 [快速上手](https://a112121788.github.io/ecode/getting-started)。
...
```powershell
git clone https://github.com/a112121788/ecode.git ECode
cd ECode
...
> Browser Surface 依赖 WebView2。部分 Windows 环境可能需要先安装 WebView2 Runtime。
...
ECode 会读取全局或项目级 `ecode.json`，并把命令显示到命令面板。读取顺序如下，后读取的本地配置会覆盖同名全局命令或 action。
...
| 设置 | `Ctrl+,` / `Ctrl+Shift+,` | 设置 / 重载 `ecode.json` |
...
- 用户文档：`docs/`，在线站点 https://a112121788.github.io/ecode/
- 设计文档：`spec/README.md`
