...
1. 启动 `ecodex-app.exe`。
2. 使用 `Ctrl+N` 选择仓库文件夹并创建 Workspace；同一文件夹只能绑定一个 Workspace。
3. 使用 `Ctrl+T` 新建 Surface，新 Terminal 默认从当前 Workspace 文件夹启动；`Ctrl+1..9` 在 Workspace 间切换。
