...
ECode 是一款面向 Windows 的原生 SuperTerminal：用 WPF + ConPTY 构建，把多项目终端、标签页、分屏、浏览器 Surface、命令日志、会话恢复和脚本化自动化放在同一个键盘优先工作台里。

- 文档站点：https://a112121788.github.io/ecode/
- 安装指南：https://a112121788.github.io/ecode/installation
- 命令行：https://a112121788.github.io/ecode/cli
- 浏览器 API：https://a112121788.github.io/ecode/browser-api

> 主程序为 `ecode-app.exe`，命令行工具为 `ecode.exe`，后台守护进程为 `ecode-daemon.exe`。
...
| 需要让外部脚本驱动终端 UI | 命名管道命令行 API 支持通知、workspace、surface、split、status | `ecode notify`、`ecode split right` |
