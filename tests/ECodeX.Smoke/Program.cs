...
internal static class Program
{
    ...
    {
        var logPath = Path.Combine(Path.GetTempPath(), "ecodex-smoke.log");
        _log = new StreamWriter(logPath, append: false) { AutoFlush = true };
        Log($"=== ECodeX ConPTY smoke (log: {logPath}) ===");
    ...
    private static async Task TestUnicodeWorkingDirectory()
    {
        ...
        var root = Path.Combine(Path.GetTempPath(), "ecodex-smoke-" + Guid.NewGuid().ToString("N"));
        ...
        try
        {
            ...
            session.Start(
                ...
                {
                    ["ECODEX_SMOKE_UNICODE"] = unicodeProjectDir,
                });
