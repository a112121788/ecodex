...
- 关闭按钮现在会保存会话并退出 `ecodex-app` 主进程，不再把主窗口隐藏到托盘。
- 最小化 ECodex 时保留任务栏按钮并继续后台运行；再次点击任务栏图标会把窗口带回前台。
- 安装版快捷方式和主进程统一使用 AppUserModelID `ECodex.App`，让固定任务栏图标、任务栏按钮、Toast 和托盘入口都激活同一个已有窗口。
- 将程序集、安装器、MSIX 清单、设置页展示和发布文档的版本号同步为 `1.0.3`。

### Fixed

- `ecodex-app` 通过 `Global\ECodexMainApp` 保持单实例；重复启动会退出新进程并请求已有窗口聚焦。
...
- 新增 AgentConversation runtime recorder Core seam，用于后续 runtime 以规范化 thread/message 事件写入可复盘会话证据；当前不接真实 LLM 或 UI。
...
- 将程序集、安装器、MSIX 清单、设置页展示和发布文档的版本号同步为 `1.0.1`。
