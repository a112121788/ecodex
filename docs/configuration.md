...
ECode 的用户配置分为三类：运行时设置、自定义命令配置和兼容开关。

## 运行时目录

默认运行时目录：`%USERPROFILE%\.ecode`
...
自定义命令配置使用 `ecode.json`。推荐路径：

1. 项目内：`.ecode/ecode.json`
2. 用户级：`%USERPROFILE%\.config\ecode\ecode.json`
...
- 不要在 `ecode.json` 中写入真实 token 或 API key。
