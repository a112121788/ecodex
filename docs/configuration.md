...
1. 用户级：`%USERPROFILE%\.config\ecodex\ecodex.json`
2. 项目内：`.ecodex/ecodex.json`
3. 项目根目录：`ecodex.json`

后读取的项目配置会覆盖同名全局命令或 action。旧 `.cmux/cmux.json` 不作为当前公开加载路径；需要迁移时请手动复制为 `.ecodex/ecodex.json`。
