...
hero:
  ...
  tagline: 为多仓库、多标签页、分屏终端、Browser Surface、自动化 API 与会话恢复设计的键盘优先 SuperTerminal。
  image:
    src: /app-icon.png
    alt: ECode 图标
  actions:
...
features:
  ...
  - title: 终端 + 浏览器协同
    details: Browser Surface 内置 WebView2，并提供 snapshot、click、fill、eval、screenshot 等自动化能力。
  - title: 项目命令可复用
...
| 用脚本控制 ECode | [CLI 参考](./cli.md) |
| 自动化浏览器面板 | [Browser API](./browser-api.md) |
...
- [Browser API](./browser-api.md)：WebView2 Browser Surface 的脚本化操作能力。
