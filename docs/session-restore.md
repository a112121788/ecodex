...
连接到 `ecodex-daemon` 时，终端进程由 daemon 托管。关闭按钮会退出 `ecodex-app` 主进程，并按 `PreserveDaemonSessionsOnClose` 设置决定是否保留 daemon 托管终端；默认保留后台终端。最小化会保持任务栏按钮并让 ECodex 在后台运行，主进程、后台终端和通知继续运行；双击托盘图标或托盘菜单“打开 ECodex”可恢复窗口。再次启动 ECodex 也会聚焦并恢复已有实例，不创建第二个长期存活的主进程。
