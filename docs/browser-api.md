...
| 3. 读取快照 | `ecodex browser snapshot --surfaceRef $surfaceRef` | 查看 role、name、text、testid。 |
| 4. 执行动作 | `ecodex browser click --role button --name Submit --surfaceRef $surfaceRef` | 用 locator 驱动页面。 |
| 5. 验证结果 | `ecodex browser eval "document.title" --surfaceRef $surfaceRef` / `ecodex browser screenshot --surfaceRef $surfaceRef` | 读取状态或保存截图。 |
...
浏览器命令通过 `surfaceRef` 定位目标。格式是 `surface:<surfaceId>`，其中 `surfaceId` 是浏览器 Surface 的真实 ID；不要把命令行 human 输出里的短引用 `surface:1` 当作 `surfaceRef`。最稳妥的做法是直接使用 `open` / `new` 的返回值：
...
ecodex browser click --testid save-button --surfaceRef $surfaceRef

# 适合文案稳定的按钮或链接
ecodex browser click --text "登录" --surfaceRef $surfaceRef
...
ecodex browser fill password "secret" --surfaceRef $surfaceRef
ecodex browser press --testid password --key Enter --surfaceRef $surfaceRef
ecodex browser eval "document.location.href" --surfaceRef $surfaceRef
...
```powershell
$shot = ecodex --json browser screenshot --surfaceRef $surfaceRef | ConvertFrom-Json
