...
- 完成 M7 文档、社区流程和发布就绪门槛。
- 保持 P0 缺陷数量 = 0，P1 缺陷数量满足 1.0 发布阈值。
- 维护 `v1.0.0` 发布说明与稳定分发指引。
...
### M1 - UI/UX 与 `ecode.json`

- Workspace、Surface、Pane 交互完善。
- 通知环、未读跳转和通知面板。
- `ecode.json` commands/actions 基础与 reload。
...
- `ecode doctor`、setup status/install/uninstall、update 命令。
...
- terminal / layout / notification / session restore / `ecode.json` / browser / v2 命令行核心流程达到 beta 或 stable 质量。
- 安装、卸载、更新流程保留 `%USERPROFILE%\.ecode` 数据。
