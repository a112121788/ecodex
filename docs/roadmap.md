...
- `S3 - 窗口关闭与单实例激活` 已随 `1.0.3` 落地：右上角 X 退出 `ecodex-app` 主进程、最小化保留任务栏并后台运行、任务栏 / 固定图标激活已有窗口。
...
| `1.0.3` | 窗口关闭与单实例激活、通知完善、默认 skills、终端多行输入 | S3 | stable |
...
- 最小化保留任务栏按钮，后台终端与通知继续运行。
- 任务栏按钮可直接恢复前台；固定任务栏图标与托盘“打开 ECodex”一致，激活已有窗口；安装器快捷方式和进程使用相同 AppUserModelID。
