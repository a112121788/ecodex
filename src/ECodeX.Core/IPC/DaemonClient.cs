...
/// <summary>
/// 连接并控制 ecodex-daemon 守护进程的客户端，负责会话生命周期管理和双向通信。
/// </summary>
public sealed class DaemonClient : IDisposable
{
    ...
    public bool TryConnect(int timeoutMs = 300)
    {
        ...
        // 先尝试新管道 `ecodex-daemon`；兼容期回退到旧管道 `cmux-daemon`。
    ...
    private static string? FindDaemonExecutable()
    {
        ...
        var candidate = Path.Combine(appDir, "ecodex-daemon.exe");
        if (File.Exists(candidate)) return candidate;

        // 2. 在兄弟项目构建输出中查找（开发构建场景）
        //    appDir 例如 .../src/ECodeX/bin/Debug/net10.0-windows10.0.17763.0/
        //    守护进程位于 .../src/ECodeX.Daemon/bin/Debug/net10.0-windows/ecodex-daemon.exe
        try
        {
            ...
            if (dir != null)
            {
                var daemonBin = Path.Combine(dir.FullName, "ECodeX.Daemon", "bin");
                if (Directory.Exists(daemonBin))
                {
                    ...
                    if (Directory.Exists(configDir))
                    {
                        ...
                        {
                            candidate = Path.Combine(tfmDir, "ecodex-daemon.exe");
                            if (File.Exists(candidate)) return candidate;
                        }
                    }

                    foreach (var exe in Directory.GetFiles(daemonBin, "ecodex-daemon.exe", SearchOption.AllDirectories))
                        return exe;
                }
            }
        }
